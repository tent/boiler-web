	...
	Boiler.Views.AppNav = React.createClass({
		...
		render: function () {
			...
			var navItems = this.props.navItems.map(function (item) {
				return <AppNavItem key={item.path} path={item.path} active={item.path === this.state.activePath} iconClassName={item.iconClassName} name={item.name} authenticated={this.props.authenticated} />;
			}.bind(this));
			return (
				<div>
